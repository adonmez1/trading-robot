...
class IELAgent(Agent):
    ...
    def __init__(self, account, email, password, marketplace_id, valuations):
    ...
    def strat_selection(self, action):
        ...
        if action == self.BUY:
            self.strategies[action] = list(
                filter(lambda x: x < self.valuations[self.get_units() + 1], self.strategies[action]))
        else:
            self.strategies[action] = list(
                filter(lambda x: x > self.valuations[self.get_units()], self.strategies[action]))
        if action == self.BUY:
    ...
    def pre_start_tasks(self):
        ...
        :return:
        """
        super().execute_periodically(self.check_book, 4)
    ...
    def send_ord(self, action, order_book):
        ...
        if action == self.BUY:
            if self.sl <= self.valuations[self.get_units() + 1]:
                ...
                if not waiting and len(my_orders["buy"]) == 0 and self.get_units() < self.max_holdings and \
                        len(self.past_trades) < 3:
            ...
            if self.su >= self.valuations[self.get_units()]:
                ...
                if not waiting and len(my_orders["sell"]) == 0 and len(self.past_trades) < 3:
...
if __name__ == "__main__":
    ...
    m_id = "market id in Flex-E-Markets"
    val1 = [0, 275, 225, 175, 125, 75, 25, 0, 0, 0, 0, 0]
    fm_bot1 = IELAgent("FM account name", "Robot user name", "password", m_id, val1)
    fm_bot1.run()
